...
import { COOKIES_SYNC_KEY, DEFAULT_ENDPOINT, HTTPTransport, IRPC_JSON_KEY } from '../src/index.js';
...
describe('HTTPRouter', () => {
  ...
  describe('resolve', () => {
    ...
    it('should safely abort response streams securely bounded to specification ttl structurally', async () => {
      ...
      const testFunc = module.declare<TestFunc>({ name: 'testTtl', seed: () => '', stream: true, ttl: 50 } as any);
    ...
    it('should bind native readable cancellation hooks securely mapped explicitly', async () => {
      ...
      type TestFunc = () => Promise<string>;
      const testFunc = module.declare<TestFunc>({ name: 'testCancel', seed: () => '', stream: true } as any);
    ...
    it('should ignore incoming payload packets dynamically mapped correctly after natively aborting readable loop', async () => {
      ...
      type TestFunc = () => Promise<string>;
      const testFunc = module.declare<TestFunc>({ name: 'testPipeAbort', seed: () => '', stream: true } as any);
    ...
    it('should use custom response builder for valid streaming requests', async () => {
      ...
      type TestFunc = () => Promise<string>;
      const testFunc = module.declare<TestFunc>({ name: 'testBuilder', seed: () => '' } as any);
  ...
  describe('resolveBuffered', () => {
    ...
    it('should return JSON response with resolved data', async () => {
      ...
      const request = createBufferedRequest({
        calls: [{ id: '1', name: 'login', args: [{ user: 'admin' }] }],
      });
    ...
    it('should return error response for non-existent function', async () => {
      ...
      const request = createBufferedRequest({
        calls: [{ id: '1', name: 'nonExistent', args: [] }],
      });
    ...
    it('should return 500 when withContext throws unexpectedly', async () => {
      ...
      // Force resolveHooks to throw, triggering the outer catch block
      vi.spyOn(router as any, 'resolveHooks').mockImplementation(() => {
    ...
    it('should replay EVENT packets into final result via RemoteState', async () => {
      ...
      const testFunc = module.declare<TestFunc>({ name: 'login', seed: () => ({ count: 0 }), stream: true } as any);
