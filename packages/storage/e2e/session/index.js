import { session, SessionStorage } from '/bundle/index.js';
// Test results will be stored here
window.testResults = {};

// Simple function to run tests and store results
window.runSessionStorageTests = async function () {
  const results = {};

  try {
    // Test 1: Initialize with initial data
    try {
      const storage = new SessionStorage('test-session-1', { a: 1, b: 'test' });
      results.test1 = {
        a: storage.get('a'),
        b: storage.get('b'),
    ...
    try {
      const storage = new SessionStorage('test-session-2', { a: 1, b: 2 });
      results.test2 = {
        length: storage.length,
      };
    } catch (error) {
    ...
    try {
      const storage = new SessionStorage('test-session-3');
      storage.set('a', 1);
      storage.set('b', 'test');
      storage.set('c', true);
    ...
    try {
      const storage = new SessionStorage('test-session-4', { a: 1, b: 2 });
    ...
    // Test 5: Assign data
    try {
      const storage = new SessionStorage('test-session-5', { a: 1 });
    ...
    try {
      const storage = new SessionStorage('test-session-6');
      const events = [];
    ...
    try {
      const storage = new SessionStorage('test-session-7', { a: 1, b: 'test' });
      results.test7 = {
        json: storage.json(),
    ...
    // Test 8: Test sessionStorage persistence
    try {
      const key = 'anchor-session://test-session-8@1.0.0';
      sessionStorage.removeItem(key); // Clean up first

      const state = session('test-session-8', { a: 1, b: 'test' });
      state.a = 2;
      state.c = 'new';
