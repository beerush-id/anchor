...
export function createPagesSync(opts: PagesSyncOptions) {
  ...
  /**
   * Scaffolds starter content into a newly created page file. Only ever
   * writes to 0-byte files — moves/copies surface as `add` too, and
   * user/IDE content must never be clobbered.
   */
  const scaffoldFile = (file: string) => {
    if (!scaffoldEnabled) return;

    const base = path.basename(file);
    if (base !== files.page && base !== files.pageMdx && base !== files.layout && base !== files.layoutMdx) return;
    ...
    try {
      if (content && fs.statSync(file).size === 0) {
        fs.writeFileSync(file, content);
      }
    } catch {}
  };

  const scaffoldAll0ByteFiles = (node: FolderNode) => {
