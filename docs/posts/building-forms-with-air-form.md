---
title: Building Forms with AIR Form
description: A comprehensive React tutorial — from a two-field form to a production-ready, multi-section form with dynamic arrays, cross-field validation, and custom inputs.
sidebar: false
next: false
prev: false
---
...
[← Back to Posts](/posts/index.html) | **`AIR Form`** **`React`**
...
configureForm({
  ...
  submit: {
    class: 'flex-1 px-4 py-2.5 text-white font-medium bg-blue-600 hover:bg-blue-700 rounded-lg shadow-sm disabled:opacity-50 disabled:cursor-not-allowed transition-all',
  },
  reset: {
    class: 'flex-1 px-4 py-2.5 text-slate-700 font-medium border border-slate-300 rounded-lg hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all',
  },
});
```
...
- [AIR Form Documentation](/airlib/form/) — Complete API reference
- [Form Inputs](/airlib/form/inputs) — All 14 built-in input components
- [Composition](/airlib/form/composition) — Cross-field matching, arrays, headless mode, and accessibility
- [Configuration](/airlib/form/configuration) — Global defaults and styling
- [Core API](/airlib/form/core-api) — Using the engine directly, without components
- [Building Smart Form Components](./building-smart-form-components) — How the form engine is built under the hood
