...
description: 'Deep-dive into IRPC CRUD declarations, adapter wiring, driver implementations, and the Chain of Responsibility.'
---
...
[Spec hooks](./handler#spec-hooks-per-function) can be attached to CRUD stubs. Pass the entire group to `irpc.hook()` and the hook runs before every method:
