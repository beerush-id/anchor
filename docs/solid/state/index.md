...
This system provides a universal, fine-grained reactivity system that works both inside and outside of SolidJS. It allows you to manage state using **standard JavaScript objects** while gaining powerful reactivity features.
...
- **Immutability**: "Don't change the object, create a new one." (React `useState`, Redux, and Solid `createStore` with `setState`). This introduces "Copy-on-Write" overhead (or complex path syntax) and disconnects state from logic.
- **Special Setters**: `setState('path', val)` or `signal()`. This adds boilerplate and makes code harder to read.
...
State in this architecture is not tied to the Solid component tree. You define your business logic and state containers (stores) using plain TypeScript functions. These "Headless States" can be tested in isolation and consumed by any UI layer (React, Vue, or Vanilla JS).
...
- **View**: Defined in the Presentation Layer using Solid Components. Handles rendering and event binding.
...
- [Immutable State](/solid/state/immutable) - Working with read-only state references.
