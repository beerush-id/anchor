...
description: "How SSR works with Solid — the app entry, client hydration, and why activation runs before hydrating."
keywords:
  - anchor solid
  - ssr
...
Solid renders the same components on the server and in the browser — no separate component trees, no `isServer` branches. The SSR pipeline itself — the `airPages()` dev server, the `createApp` worker, static generation, caching, and running on each runtime — is covered in [Universal SSR](/ssr). This page covers the Solid side: the two entry points and what happens between the server render and the browser.
...
router
  ...
  .then(() => hydrate(() => <App />, document.getElementById('root')!))
