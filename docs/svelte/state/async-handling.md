...
description: "Managing asynchronous operations with reactive state in Svelte."
keywords:
  - async
  - query
  - fetch
...
Anchor provides complementary approaches for async operations that work seamlessly with Svelte:
...
<div>
  <p>Status: {userQuery.status}</p>
  {#if userQuery.data}
    <p>Data: {userQuery.data.name}</p>
  {/if}
</div>
```
...
<button on:click={() => dataQuery.start()}>Refresh</button>
```
...
```svelte
<script lang="ts">
  import { mutable, query, effect } from '@anchorlib/svelte';
...
<input bind:value={term.value} placeholder="Search..." />
```
...
```svelte
<script lang="ts">
  import { mutable, query } from '@anchorlib/svelte';
