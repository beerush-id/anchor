...
### Cons
- **SSR Risk (Critical)**: In a Server-Side Rendering (SSR) environment like SvelteBar / SvelteKit, module-level variables are shared across **all requests**. If one user changes `appState.theme`, *every* concurrent user on that server instance sees the change. This creates data leaks and race conditions.
...
- **Alternative**: For SSR-safe global state, use [Svelte Context](https://svelte.dev/docs/svelte#setcontext) or a Dependency Injection system to scope the state to the request tree.
...
For more complex logic, Classes offer better structure and inheritance.
...
Local state is state that is created and destroyed with a specific component instance. In Svelte, this is simply defined inside the component script.
...
<script lang="ts">
  ...
  // The script runs when the component is instantiated.
