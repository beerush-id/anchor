---
title: "Derived State"
description: "Understanding the concept of computed state and single source of truth in Vue."
keywords:
  - derived state
  - computed
...
In these cases, you define a **Reactive Computation** that combines these sources using `derivedRef`.

```ts
import { anchorRef, derivedRef } from '@anchorlib/vue';

const todos = anchorRef([{ text: 'Buy milk', done: false }]);
const filter = anchorRef('SHOW_ALL');

// This value is computed from two independent sources: 'todos' and 'filter'
const visibleTodos = derivedRef(() => {
  if (filter.value === 'SHOW_COMPLETED') return todos.value.filter(t => t.done);
  return todos.value;
});
```
...
| **Composite** | `derivedRef` function | Combining separate states (`Search` + `List`) or View Models. |
