...
### Cons
- **SSR Risk (Critical)**: In a Server-Side Rendering (SSR) environment like Next.js or Remix, module-level variables are shared across **all requests**. If one user changes `appState.theme`, *every* concurrent user on that server instance sees the change. This creates data leaks and race conditions.
...
- **Alternative**: For SSR-safe global state, use [React Context](/react/component/binding#context-binding) or a Dependency Injection system to scope the state to the request tree.
