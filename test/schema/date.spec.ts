...
import { DateSchema, satisfy, Schema, SchemaType, validate } from '../../lib/esm/schema';
...
test('validates date width default value string', () => {
  ...
  const value = satisfy(schema as Schema<Date>);
  assert(value instanceof Date, 'Expected value to be a date');

  const result = validate(schema as Schema<Date>, value);
  assert(result.valid, 'Expected value to be set and valid');
});
