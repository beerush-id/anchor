...
|---|---|---|
| `form` | `class`, `errorClass`, `pendingClass`, `strict`, `validateOnInit`, `settleOnSubmit` | Configuration for the main `<Form>` container. |
| `field` | `class`, `labelClass`, `controlClass`, `supportClass`, `errorClass`, `requiredLabel`, `requiredClass`, `mismatchLabel` | Configuration for the `<Field>` wrapper, including its label, control container, and support/error display. |
...
| `reset` | `class`, `dirtyClass` | Default classes for `<FormReset>` buttons. |
