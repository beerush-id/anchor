---
title: 'Core API'
description: 'Use the AirLib Form engine directly — formState, formField, formInput — without framework-specific components.'
nav: 'Core API'
nav-priority: 116
---
...
The `formState` function creates the reactive form store from a Zod schema. This is the foundation — all field states, validation, change tracking, and submission lifecycle live here.
...
The `formField` function creates a reactive reference to a single field within the nearest form context. When used inside a component tree with an active form provider, it connects to that form.
...
The `formInput` function creates an input controller that handles string buffering and type conversion. Useful for building input components that need to bridge between display strings and typed values.
...
The `formFactory` function wraps `formState` with a factory pattern for reusable, typed form creation. Useful when the same schema is used across multiple components.
...
- [Getting Started](/docs/form/getting-started) — Build forms with components
