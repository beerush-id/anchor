---
title: 'Composition'
description: 'Cross-field matching, array fields, headless rendering, custom inputs, and accessibility in AirLib Form.'
nav: 'Composition'
nav-priority: 113
---
...
The [Getting Started](/docs/form/getting-started) page covers the basics: schema, fields, validation, submit. This page covers the patterns that emerge when forms get complex — matching fields, dynamic arrays, custom rendering, and building your own inputs.
...
<UserForm.Field name="email">
  {(field) => (
    <div>
      <EmailInput />
      <Show when={() => field.touched && !field.valid}>
...
```tsx [React] name="react"
import { setup, render, mutable, Show } from '@airlib/react';
...
export const AsyncUsernameInput = setup(() => {
  ...
  return render(() => (
    <div>
      ...
      <Show when={() => state.taken}>
        {() => <span className="error">This username is already taken</span>}
      </Show>
    </div>
  ));
});
...
```tsx [SolidJS] name="solid"
import { setup, mutable, Show } from '@airlib/solid';
...
```tsx [React] name="react"
import { Show, For } from '@airlib/react';
    ...
    <div className="custom-field">
      ...
      <div className="field-meta">
        ...
        <Show when={() => !field.matched}>
          {() => <span className="warning">Doesn't match</span>}
        </Show>
      </div>
    </div>
  )}
...
Both `Form` and `Field` support declarative **named component slots** via `<Component.Snippet>` to customize structural layout regions without giving up built-in styling or state wiring.

### Field Slots

Inject prefixes, suffixes, custom labels, or specialized error renderers into any field:

:::code-group{group="framework"}

```tsx [React] name="react"
<UserForm.Field name="email" label="Email">
  <UserForm.Field.Snippet for="prefix">
  ...
  <UserForm.Field.Snippet for="support">
    {(field) => (
      <span className="text-xs text-muted">
        We will send verification instructions here.
      </span>
    )}
  </UserForm.Field.Snippet>
</UserForm.Field>
...
|------|-----------|-------------|
| `label` | `field` | Customizes or replaces the field label element |
| `prefix` | `field` | Renders leading content/icons directly before the input control |
| `suffix` | `field` | Renders trailing content/icons directly after the input control |
| `support` | `field` | Customizes the support region below the control (defaults to validation errors and mismatch alerts) |

### Form Slots

Inject custom banners, headers, footers, or action bars directly inside the form boundaries:
...
<UserForm onSubmit={save}>
  <UserForm.Snippet for="header">
    {(form) => (
      <header className="form-header">
        ...
        <Show when={() => form.changed}>
          {() => <span className="badge">Unsaved changes</span>}
        </Show>
      </header>
    )}
  </UserForm.Snippet>
  ...
  <UserForm.Snippet for="actions">
    {(form) => (
      <div className="flex gap-2">
        <UserForm.Reset>Reset</UserForm.Reset>
        <UserForm.Submit className="flex-1">
...
Available `<Form>` slots:

| Slot | Parameter | Description |
|------|-----------|-------------|
| `header` | `form` | Renders at the top of the form, before fields |
| `error` | `form` | Customizes or replaces the top-level form error banner |
| `actions` | `form` | Renders action controls below the fields |
| `footer` | `form` | Renders at the bottom of the form boundary |
...
### Headless Input Customization

All built-in inputs (`Checkbox`, `Radio`, `TextInput`, `Select`, `Textarea`, `FilePicker`) accept a function as `children`. When provided, the input prepares all necessary HTML properties, event handlers, IDs, and ARIA attributes and yields `(props, input)` so you can easily spread `{...props}` onto your custom markup:

:::code-group{group="framework"}

```tsx [React] name="react"
import { Checkbox, Radio, TextInput } from '@airlib/react-form';
import { classx, Show } from '@airlib/react';

// Custom Search Box with Inner Icon
<TextInput placeholder="Search users...">
  {(props, input) => (
    <div className="custom-input-box">
      <span className="search-icon">🔍</span>
      <input {...props} />
      <Show when={() => input.touched && input.error}>
...
<Checkbox name="notifications">
  {(props, input) => (
    <label className={classx('custom-switch', { active: input.checked })}>
...
<Radio value="pro">
  {(props, input) => (
    <label className={classx('plan-card', { active: input.checked })}>
      <input {...props} className="sr-only" />
      <h4>Pro Plan</h4>
      <span>$19/mo</span>
    </label>
...
```tsx [SolidJS] name="solid"
import { Checkbox, Radio, TextInput } from '@airlib/solid-form';
...
<Checkbox name="notifications">
  {(props, input) => (
    <label class={classx('custom-switch', { active: input.checked })}>
...
<Radio value="pro">
  {(props, input) => (
    <label class={classx('plan-card', { active: input.checked })}>
      <input {...props} class="sr-only" />
      <h4>Pro Plan</h4>
      <span>$19/mo</span>
    </label>
...
### Building Reusable Custom Inputs

Use `for={props}` to wrap built-in inputs inside custom reusable components without writing any manual event or state wiring:

:::code-group{group="framework"}

```tsx [React] name="react"
import { setup, Show } from '@airlib/react';
import { TextInput, TextInputProps } from '@airlib/react-form';

export const SearchInput = setup<TextInputProps>((props) => (
  <TextInput for={props}>
    {(inputProps, input) => (
...
import { Show } from 'solid-js';
import { TextInput, TextInputProps } from '@airlib/solid-form';
...
- [Core API](/docs/form/core-api) — Use the engine directly, without components
- [Getting Started](/docs/form/getting-started) — Back to the basics
