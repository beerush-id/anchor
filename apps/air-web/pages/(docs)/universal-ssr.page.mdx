...
nav-priority: 50
title: "Universal SSR"
description: "The SSR pipeline — dev server, production worker, static generation, on-demand pre-rendering, and caching across Bun, Node.js, Cloudflare, and Deno."
keywords:
  - anchor
  - ssr
...
This page covers the SSR pipeline end to end: the dev server, the production worker, running it on each runtime, and making pages static. The basics of building an app — layouts, pages, guards, providers, routing — are in [Getting Started](/docs/getting-started).
...
- `manifest` and `metadata` generate the [route manifest](/docs/getting-started#building-navigation) and MDX frontmatter data.
...
```tsx
// src/client.tsx
import '@airlib/react/client'; // must be the first import
...
- `acceptInteractions()` is optional — it enables the live browser states (see [Browser Utilities](/docs/user-interface/browser)). Without it the page works; the live objects just stay off.
:::
...
The [Remote Function](/docs/remote-function) guide covers IRPC routing in depth.
