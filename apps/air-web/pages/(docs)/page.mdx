...
nav-priority: 10
title: 'Overview'
description: 'AirLib combines Anchor, IRPC, and Router into one reactive full-stack TypeScript framework — fine-grained state, isomorphic remote functions, type-safe routing, and universal SSR.'
keywords:
  - AirLib
  - Anchor
  - IRPC
...
With AirLib, you don't need to install a million libraries to build an app that is **highly reactive**, **performant**, and **maintainable**—all **without making sacrifices**.
...
```tsx [React]
import { setup, mutable, effect, Show } from '@airlib/react';
import { LIVE_SELECTION as selection, LIVE_KEYBOARD as key } from '@airlib/react/browser';
...
```tsx [Solid]
import { setup, mutable, effect, Show } from '@airlib/solid';
import { LIVE_SELECTION as selection, LIVE_KEYBOARD as key } from '@airlib/solid/browser';
