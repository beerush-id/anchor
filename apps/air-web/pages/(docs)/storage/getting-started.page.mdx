...
description: 'Anchor storage module with reactive storage backends — IndexedDB, Session Storage, and Local Storage.'
nav: 'Getting Started'
nav-icon: storage
nav-priority: 200
---
...
The storage module is part of the `@airlib/storage` package:

:::code-group

```sh [Bun]
bun add @airlib/storage
```
...
```sh [Yarn]
yarn add @airlib/storage
```

```sh [PNPM]
pnpm add @airlib/storage
```
