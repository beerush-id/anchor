...
description: 'Reactive localStorage persistence with the persistent() function — two-way sync, versioning, and lifecycle management.'
nav: 'Persistent Storage'
nav-priority: 202
---

# Persistent Storage

The Persistent Storage functionality is provided through the **`persistent`** function, which creates reactive objects that automatically synchronize with `localStorage`. This allows you to work with persistent data as if it were regular JavaScript objects while having changes automatically saved.

:::tip[Two Way Binding]

Anchor stores the data in `localStorage` and automatically `syncs` it with the reactive object. When the engine detect a change made from another browser tab, the object will be updated.
