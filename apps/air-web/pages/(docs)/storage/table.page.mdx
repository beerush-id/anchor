---
title: 'Table Store'
description: 'Structured IndexedDB storage with indexing, complex queries, and reactive state management.'
nav: 'Table Store'
nav-priority: 205
---
...
The Table Store in Anchor provides a structured storage solution backed by IndexedDB with support for complex queries, indexing, and relational-like operations. It's designed for scenarios where you need to store collections of structured data with advanced querying capabilities.

## Overview

The Table Store functionality is provided through the **`createTable`** function. It offers an optimistic API for IndexedDB operations with built-in support for indexing, querying, and reactive state management.
