...
description: 'Reactive sessionStorage persistence with the session() function — temporary data that survives navigation within a tab.'
nav: 'Session Storage'
nav-priority: 203
---
...
The Session Storage functionality is provided through the **`session`** function, which creates reactive objects that automatically synchronize with `sessionStorage`. This is ideal for temporary data that should persist during navigation within the same tab but not across different sessions.
