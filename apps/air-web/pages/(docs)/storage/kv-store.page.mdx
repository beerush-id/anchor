---
title: 'KV Store'
description: 'Key-value storage backed by IndexedDB with optimistic concurrency control and reactive state.'
nav: 'KV Store'
nav-priority: 204
---
...
The KV Store functionality is provided through the **`createKVStore`** function. It offers an optimistic storage model where operations are performed in-memory first and then synchronized with IndexedDB in the background.
...
```typescript
import { createKVStore } from '@airlib/storage/db';
