...
```tsx [React]
// page.tsx
import { page, Show, For } from '@airlib/react';
...
```tsx [SolidJS]
// page.tsx
import { page, Show, For } from '@airlib/solid';
import { profileRoute } from './route.js';
...
```ts [React]
// lib/router.ts
import { createRouter, MAX_AGE } from '@airlib/react';
...
```ts [SolidJS]
// lib/router.ts
import { createRouter, MAX_AGE } from '@airlib/solid';
...
- [Routes & Layouts](/docs/routing/routes-layouts) — route tree, index routes, render API, route state, and options
- [Navigation](/docs/routing/navigation) — `<Link>`, `navigate()`, active state, and preloading
- [Data Loaders](/docs/routing/data-loaders) — `.provide()`, caching, retry, and reactive re-evaluation
- [Guards](/docs/routing/guards) — `.guard()`, `redirect()`, error handling, and route protection
