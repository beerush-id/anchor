...
# IRPC vs. Hono :badge[Comparison]{variant="info"} :badge[Web Standards]{variant="neutral"}

[← Back](./) | **Technical Comparison** | **`Hono`** **`hono/client`** **`RPC`**

<a href="https://hono.dev/" target="_blank" rel="noreferrer">Hono</a> is a lightweight, ultrafast web framework built on Web Standards. It's famous for running anywhere (Cloudflare Workers, Deno, Bun) and provides end-to-end type safety using its `hono/client` RPC client. Your frontend automatically infers the types of your backend endpoints without code generation.

IRPC (part of AirLib) achieves end-to-end type safety without relying on HTTP routing semantics. Instead of inferring types from URL paths and HTTP verbs, you explicitly declare pure function signatures. 
...
import { construct } from '@irpclib/irpc';
import { sayHello } from './declare.js';

// Implement the contract on the server
construct(sayHello, (name = 'World') => {
  return `Hello, ${name}!`;
});
```
...
```typescript
// client.ts
import { sayHello } from './rpc/hello/declare.js';

async function main() {
  // Call the contract directly
  const greeting = await sayHello('Alice');
  console.log(greeting); // Hello, Alice!
}
```

There is no routing boilerplate, no HTTP method selection, and no query parameter mapping. You invoke the function directly.

## 3. UI Integration (React)

How endpoints bind to UI state defines your daily developer experience.

### Calling Hono in React

Because Hono's RPC client returns raw `Response` objects, you must integrate it with a fetching library like `@tanstack/react-query` to handle loading and error states.
...
You don't make explicit network requests. You bind the stub to your reactive UI, and AirLib handles the loading state, caching, and reactivity natively.
