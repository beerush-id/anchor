...
```tsx [React]
import { setup, derived } from '@airlib/react';
...
const PriceTag = setup<{ amount: number; currency?: string }>((props) => {
  ...
  return () => <span className="price">{ display.value }</span>;
});
```
...
- Returning an **accessor `(props) => ()`** creates a **reactive view** that automatically tracks signals/properties, receives `props` as its argument (enabling direct destructuring like `return ({ children }) => (...)`), and re-renders independently when accessed state changes.
...
```tsx [React]
import { setup, mutable, onMount, onCleanup } from '@airlib/react';
...
```tsx [React]
import { setup, mutable, onCleanup, type Bindable } from '@airlib/react';
...
When a component needs **early returns**, **multi-branch view switching**, or **render-prop evaluation**, returning an accessor `(props) => JSX` and using **`renderDynamic()`** handles it cleanly:

- **`(props) => JSX` (Reactive Accessor)**: Declares a reactive view body that automatically re-evaluates whenever any state or props accessed inside it change. The accessor receives the component's `props` as its argument, enabling clean destructuring (e.g. `return ({ children }) => (...)`). This allows conditional returns and branch switching in both React and SolidJS without extra markup wrappers.
...
```tsx [React]
import { setup, renderDynamic, type Bindable } from '@airlib/react';

const StatusCard = setup<{
  loading?: Bindable<boolean>;
  children?: ((status: string) => JSX.Element) | JSX.Element;
}>((props) => {
  // The accessor receives props as its argument, allowing direct destructuring
...
```tsx [SolidJS]
import { setup, renderDynamic, type Bindable } from '@airlib/solid';
...
Using reactive accessors `(props) => JSX` and **`renderDynamic()`** for reactive conditional returns and dynamic render props.

**From these two examples, you know:**

- Returning an accessor **`(props) => JSX`** enables conditional returns and branching directly in the view function while preserving full reactivity
- The accessor receives **`props` as its argument**, allowing clean destructuring directly in the return signature (e.g. `return ({ children }) => (...)`)
...
```tsx [React]
import { setup, Slot, Snippet } from '@airlib/react';
...
```tsx [React]
import { setup, Slot, Snippet, mutable } from '@airlib/react';
import type { ReactNode } from 'react';
...
```tsx [React]
import { setup, derived, type Bindable } from '@airlib/react';
