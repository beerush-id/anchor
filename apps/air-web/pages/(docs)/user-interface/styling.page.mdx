...
nav-priority: 101
title: 'UI Styling'
description: 'UI styling in the AirLib — theming, inline classes, local factories, utility classes, and plain CSS, with guidance on when each fits.'
---
...
### Using `classx`
To seamlessly manage conditional classes without manually writing switch statements or bloated ternaries, the `@airlib/core` library provides the `classx` utility. It accepts strings, arrays, or objects (where keys are classes and values are conditions).
...
Managing inline styles dynamically often requires tedious string concatenation for CSS units. The `@airlib/core` library provides the `stylex` utility to automatically format units and handle reactive style objects gracefully.
