...
nav-priority: 20
title: 'Installation'
description: 'Install AirLib via starter templates or package managers — core packages, React and SolidJS integrations, and project conventions.'
keywords:
  - AirLib installation
  - Anchor installation
  - IRPC installation
...
The AirLib is modular. You can start with our official templates or install only the layers you need for your application.
...
```
```bash [NPX]
npx degit beerush-id/airlib/templates/react-ssr my-ssr-app
```
:::
...
```
```bash [NPX]
npx degit beerush-id/airlib/templates/irpc-bun-starter my-api
```
:::

## Project Structure & Conventions

When using the templates or building a full-stack AirLib application, we recommend the following file and folder conventions to keep your codebase organized:
...
```bash [Bun]
bun add @airlib/core @airlib/router @irpclib/irpc @irpclib/http
```

```bash [NPM]
npm install @airlib/core @airlib/router @irpclib/irpc @irpclib/http
```

```bash [Yarn]
yarn add @airlib/core @airlib/router @irpclib/irpc @irpclib/http
```

```bash [PNPM]
pnpm add @airlib/core @airlib/router @irpclib/irpc @irpclib/http
```
...
```bash [NPM]
npm install @airlib/core @airlib/router @airlib/react
```

```bash [Yarn]
yarn add @airlib/core @airlib/router @airlib/react
```

```bash [PNPM]
pnpm add @airlib/core @airlib/router @airlib/react
```
...
```bash [NPM]
npm install @airlib/core @airlib/router @airlib/solid
```

```bash [Yarn]
yarn add @airlib/core @airlib/router @airlib/solid
```

```bash [PNPM]
pnpm add @airlib/core @airlib/router @airlib/solid
```
...
- [Getting Started](/docs/getting-started) - Build your first application using the full AirLib.
- [Router](/docs/routing) - Set up your URL and state-driven routing.

## Need Help?

If you encounter issues during installation:

1. Open an issue on [GitHub](https://github.com/beerush-id/airlib/issues)
2. Join our community Discord for real-time support
