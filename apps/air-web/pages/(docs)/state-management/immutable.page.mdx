...
description: "Safe state sharing with read-only contracts — immutable views, write contracts, and least-privilege writers."
keywords:
  - immutable
  - read-only
  - contracts
...
```ts [React]
import { immutable } from '@airlib/react';
...
```ts [SolidJS]
import { immutable } from '@airlib/solid';
