---
nav: 'AirLib Form vs. React Hook Form'
title: 'AirLib Form vs. React Hook Form'
description: 'Comparing declarative schema-driven components with imperative register calls.'
date: '2026-06-28'
category: 'AirLib Form Posts'
keywords:
  - AirLib Form vs React Hook Form
  - React Hook Form comparison
  - React forms
---

# AirLib Form vs. React Hook Form :badge[Comparison]{variant="info"} :badge[React]{variant="neutral"}

[← Back](./) | **Technical Comparison** | **`React Hook Form`** **`AirLib Form`**

<a href="https://react-hook-form.com/" target="_blank" rel="noreferrer">React Hook Form</a> (RHF) is one of the most popular form libraries in the React ecosystem. It gained popularity by reducing renders compared to older libraries (like Formik) through uncontrolled inputs and refs.

**AirLib Form** takes a fundamentally different approach. It builds on Anchor's fine-grained reactivity graph, meaning it doesn't need to bypass React's render lifecycle with uncontrolled inputs to achieve performance. Instead, AirLib Form provides schema-driven, autonomous components that manage themselves.
...
| Aspect | React Hook Form | AirLib Form |
|---|---|---|
...
In AirLib Form, the schema *is* the form. You pass the schema to `createForm`, which returns a namespace of components (`Form`, `Field`, etc.) that are already wired to the schema's types and rules.
...
### AirLib Form's Fine-Grained Binding

Because AirLib Form is built on Anchor's reactive proxy engine, the form state is a reactive proxy. You don't need special `watch` hooks; you just read the state. Only the DOM node observing that specific state re-renders.
...
### AirLib Form's Standalone Inputs

In AirLib Form, any component can become a form input by accepting a `Bindable` value. You don't wrap them; you just bind them. In fact, if placed inside a `<Form>`, they auto-detect their context.
...
Because AirLib Form state is a mutable proxy, there are no special hooks or array helpers. You just mutate the array directly using standard JavaScript array methods.

```tsx
import { setup, render, mutable, For } from '@airlib/react';
...
**AirLib Form**, conversely, requires zero behavioral configuration. Because it inherently understands Zod and relies on the reactive proxy for state, there are no adapters or modes to configure. You pass the schema to `createForm`, and the form autonomously knows how to validate continuously.
...
| Dimension | React Hook Form | AirLib Form |
|-----------|-----------------|----------|
| **Input Registration** | Imperative `register('name')` | :badge[Declarative & Auto-Wiring]{variant="tip"} |
| **Validation Engine** | External resolver adapters (`zodResolver`) | :badge[Native Schema Integration]{variant="tip"} |
| **Custom Inputs** | `<Controller>` wrappers required | :badge[Headless `<Field>` & Autonomous Inputs]{variant="tip"} |
| **Global Styling** | Manual component wrappers | :badge[`configureForm()` Engine-Level Theme]{variant="tip"} |
| **Array Mutations** | `useFieldArray` hook ceremony | :badge[Native JS Array Methods on Proxy]{variant="tip"} |

## Final thoughts

React Hook Form and AirLib Form are both highly performant, but their architectures cater to different mental models.
...
**Choose AirLib Form if:** You prefer a declarative, schema-first approach where components govern themselves. If you want two-way binding without the performance penalty of virtual DOM diffing, and hate writing `register()` and `<Controller>` boilerplate for every custom input, AirLib Form's reactive engine is superior.
