...
const DemoTodoApp = setup(function TodoApp() {
  const newText = mutable<string>('');
  const todos = mutable<Todo[]>([]);
  const items = derived(() => todos.filter((item) => !item.deleted_at));
      ...
      <div className="todo-form mb-6">
        <div className="flex gap-2">
          ...
          <button
            ...
            disabled={!canSubmit}
            className="rounded-lg bg-blue-500 px-4 py-2 font-bold text-white transition duration-200 hover:bg-blue-700 disabled:opacity-50"
          >
            <PlusIcon />
          </button>
          <span>Rendered: {formRenderCount} times</span>
        </div>
      </div>
    );
  }, 'TodoForm');
...
const TodoItem = setup(({ todo }: { todo: Todo }) => {
  const handleToggle = () => {
