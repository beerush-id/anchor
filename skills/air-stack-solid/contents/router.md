...
interface Router {
  ...
  catch(renderer: (props: { error: RouteError }) => JSX.Element): void;
...
interface Route {
  ...
  render(renderer: (props: { state, context, children }) => JSX.Element): Route;
  renderAsync(loader: () => Promise<(props: { state, context, children }) => JSX.Element>, fallback?: (props: { state, context, children }) => JSX.Element): Route;
  catch(renderer: (props: { error, state, context }) => JSX.Element): void;
}
...
function Show<T>(props: { when: T | (() => T); children: JSX.Element | ((value: T) => JSX.Element); fallback?: () => JSX.Element }): JSX.Element;

// List rendering
function For<T>(props: { each: T[] | (() => T[]); children: (item: T, index: number) => JSX.Element; fallback?: JSX.Element }): JSX.Element;
...
render(() => <UIRouter router={router} root={RootLayout} resetScroll />, document.body);
```
...
router.activate(window.location.href).then(() => {
  ...
  root.innerHTML = '';
  render(() => <UIRouter router={router} root={RootLayout} resetScroll />, root);
});
```
...
```tsx
export const SettingsLayout = page(settingsRoute).render(({ state, children }) => {
  // If the guard or provider threw an Error, render the error barrier directly
  if (state.error) {
    return <div class="error">Failed: {state.error.message}</div>;
  }
      ...
      {({ name, email }) => (
        <div>
          ...
          <Show when={state.resolving.has('notifications')}>
            <span class="spinner">Updating notifications...</span>
          </Show>
        </div>
      )}
    </Show>
...
export const ProfilePage = page(profileRoute).renderAsync(
  async () => {
    ...
    // Pass state directly, it's inherently fine-grained.
    return ({ state }) => <ProfileComponent profile={state.data.profile} />;
  },
  ...
  ({ state }) => <div class="skeleton-ui">Loading profile for user {state.params.user_id}...</div>
);
```
