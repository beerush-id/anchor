...
Read global state singletons directly inside `<Show>` or `effect()` boundaries. Do not create custom component wrappers just to read a global singleton — import only the specific singleton you need.
...
export const InputTracker = setup(() => {
  ...
  return <input ref={tracker} placeholder="Type here..." />;
});
```
  ...
  return (
    <>
      ...
      <Show when={() => LIVE_DND.isDragging && LIVE_DND}>
        {(droppable) => (
          <div ref={(el) => droppable(el)} class="drop-zone">
            Drop here
          </div>
        )}
      </Show>
...
export const AppHeader = setup(() => {
  return <header class={LIVE_SCROLL.y > 0 ? 'shadow-md' : ''}>Header</header>;
});
```
