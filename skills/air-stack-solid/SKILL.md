...
- **When tools**: Using `degit` templates, `@airlib/solid` packages, folder structure (`function.ts`, `constructor.ts`, `route.ts`).
...
- **When tools**: Using `createWorker()`, `createFullWorker()`, `vite-ssr` plugin. Configuring `@airlib/vite`, `wrangler.toml`. Server entry points per runtime.
...
- **When tools**: Importing from `@airlib/solid/browser`. Using `LIVE_KEYBOARD`, `LIVE_CLIPBOARD`, `LIVE_DND`, `LIVE_CURSOR`, `LIVE_SCROLL`, `LIVE_SELECTION`, `LIVE_MEDIA`, `LIVE_WINDOW`, `LIVE_NETWORK`, `LIVE_GEO`, `LIVE_LOCATION`, `cursorRef()`, `scrollRef()`, `keyboardRef()`, `reframe()`, `acceptInteractions()`.
...
- **When tools**: Using `$bind()`, `Bindable<T>`, form context. `@airlib/solid` field components.
