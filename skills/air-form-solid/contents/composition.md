  ...
  return (
    <SignUpForm onSubmit={handleSubmit}>
      ...
      >
        ...
        <Show when={taken.status === 'pending'}>
  ...
  {(field) => (
    <div class="custom-field">
      ...
      <div class="field-meta">
        ...
        <Show when={!field.matched}>
          <span class="warning">Doesn't match</span>
        </Show>
      </div>
    </div>
  )}
...
Note: In SolidJS `<For>`, the index `i` is an accessor function `i()`, so we call it when building dynamic property paths or using it in callbacks.
